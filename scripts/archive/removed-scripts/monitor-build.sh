#!/bin/bash
# monitor-build.sh
# Unified build progress monitor combining features from all monitor scripts
# This is the recommended monitoring script

ANDROID_SDK="$LOCALAPPDATA/Android/Sdk"
ADB_PATH="$ANDROID_SDK/platform-tools/adb.exe"

# Configuration
UPDATE_INTERVAL=15  # seconds

# Colors
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
RED='\033[0;31m'
NC='\033[0m'

# Check if emulator is running
check_emulator() {
    if ! "$ADB_PATH" devices | grep -q "emulator"; then
        echo -e "${RED}✗ No emulator detected${NC}"
        exit 1
    fi
}

# Main monitoring loop
monitor() {
    while true; do
        clear
        echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${BLUE}║     DROIDRUN[GOOGLE] BUILD PROGRESS MONITOR                  ║${NC}"
        echo -e "${BLUE}║     $(date '+%Y-%m-%d %H:%M:%S')                                    ║${NC}"
        echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        
        "$ADB_PATH" shell "run-as com.termux sh -c 'export PREFIX=/data/data/com.termux/files/usr && export HOME=/data/data/com.termux/files/home && export PATH=\$PREFIX/bin:\$PATH && 
        
        echo \"📦 INSTALLED PACKAGES:\"
        echo \"────────────────────────────────────────────────────────────\"
        pip list 2>/dev/null | grep -E \"numpy|scipy|pandas|scikit-learn|jiter|pyarrow|psutil|droidrun|Cython|meson-python|pythran|pybind11|maturin\" | awk \"{printf \\\"  ${GREEN}✓${NC} %-20s %s\\n\\\", \\\$1, \\\$2}\" || echo \"  (checking...)\"
        echo \"\"
        
        echo \"🔨 BUILD STATUS:\"
        echo \"────────────────────────────────────────────────────────────\"
        if [ -f \$HOME/wheels/build-monitor.log ]; then
            echo \"  Recent log entries:\"
            tail -8 \$HOME/wheels/build-monitor.log | sed \"s/^/  /\"
        else
            echo \"  Waiting for build to start...\"
        fi
        echo \"\"
        
        echo \"⚙️  ACTIVE BUILD PROCESSES:\"
        echo \"────────────────────────────────────────────────────────────\"
        BUILD_PROCS=\$(ps aux | grep -E \"pip|python.*wheel|ninja|meson|clang.*scipy|rustc\" | grep -v grep | grep -v monitor)
        if [ -n \"\$BUILD_PROCS\" ]; then
            echo \"\$BUILD_PROCS\" | head -5 | awk \"{printf \\\"  → %-12s %s %s %s %s %s\\n\\\", \\\$1, \\\$11, \\\$12, \\\$13, \\\$14, \\\$15}\"
        else
            echo \"  No active build processes\"
        fi
        echo \"\"
        
        echo \"📁 WHEEL FILES:\"
        echo \"────────────────────────────────────────────────────────────\"
        WHEEL_COUNT=\$(ls -1 \$HOME/wheels/*.whl 2>/dev/null | wc -l)
        echo \"  Total wheels built: \$WHEEL_COUNT\"
        ls -lh \$HOME/wheels/*.whl 2>/dev/null | tail -5 | awk \"{printf \\\"  %-45s %6s\\n\\\", \\\$9, \\\$5}\" | sed \"s|.*/||\" || echo \"  (wheels being built...)\"
        echo \"\"
        
        echo \"💾 SYSTEM RESOURCES:\"
        echo \"────────────────────────────────────────────────────────────\"
        free -h | grep Mem | awk \"{printf \\\"  Memory: %s / %s (%s free, %s used)\\n\\\", \\\$3, \\\$2, \\\$4, \\\$7}\"
        echo \"\"
        
        echo \"📊 PROGRESS SUMMARY:\"
        echo \"────────────────────────────────────────────────────────────\"
        COMPLETED=\$(pip list 2>/dev/null | grep -E \"^numpy|^scipy|^pandas|^scikit-learn|^jiter|^pyarrow|^psutil|^droidrun\" | wc -l)
        echo \"  Packages installed: \$COMPLETED / 8\"
        if [ \$COMPLETED -eq 8 ]; then
            echo \"  ${GREEN}✅ ALL PACKAGES INSTALLED!${NC}\"
        elif [ \$COMPLETED -ge 2 ]; then
            echo \"  ${YELLOW}⏳ Build in progress...${NC}\"
        else
            echo \"  ${BLUE}🚀 Starting build...${NC}\"
        fi
        '"
        
        echo ""
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        echo "Next update in ${UPDATE_INTERVAL} seconds... (Press Ctrl+C to stop)"
        sleep "$UPDATE_INTERVAL"
    done
}

# Main
check_emulator
monitor

